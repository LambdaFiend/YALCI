...
eval1 t = let tm = getTm t; fi = getFI t in
  ...
  case tm of
    ...
    TmIf t1 t2 t3 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmIf result t2 t3
    TmSucc t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmSucc result
    TmPred (TermNode _ TmZero) -> TmZero
    TmPred (TermNode _ (TmSucc nv1)) | isVal nv1 -> getTm nv1
    TmPred t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmPred result
    TmIsZero (TermNode _ TmZero) -> TmTrue
    TmIsZero (TermNode _ (TmSucc nv1)) | isVal nv1 -> TmFalse
    TmIsZero t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmIsZero result
    TmSeq (TermNode _ TmUnit) t2 -> getTm t2
    TmSeq t1 t2 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmSeq result t2
    TmAscribe t1 ty | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmAscribe result ty
    TmAscribe v1 _ -> getTm v1
    TmLet p t1 t2 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $  TmLet p result t2
    TmLet p v1 t2 ->
      let subs = reverse $ match p v1; result = foldr ($) t2 subs
       in if elem (TermNode noPos $ TmErr "Matching error") $ map (\x -> x $ TermNode noPos TmTrue) subs
    ...
    TmVariant x t1 ty | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmVariant x result ty
    TmCase t1 ts1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmCase result ts1
    ...
    TmFix t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmFix result
    TmCons ty v1 t2 | isVal v1 ->
      let result = eval1 t2
       in checkError result $ TmCons ty v1 result
    TmCons ty t1 t2 ->
      let result = eval1 t1
       in checkError result $ TmCons ty result t2
    TmIsNil _ (TermNode _ (TmNil _)) -> TmTrue
    TmIsNil _ (TermNode _ (TmCons _ v1 v2)) | isVal v1 && isVal v2 -> TmFalse
    TmIsNil ty t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmIsNil ty $ eval1 t1
    TmHead _ (TermNode _ (TmCons _ v1 v2)) | isVal v1 && isVal v2 -> getTm v1
    TmHead ty t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmHead ty result
    TmTail _ (TermNode _ (TmCons _ v1 v2)) | isVal v1 && isVal v2 -> getTm v2
    TmTail ty t1 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmTail ty result
    TmTyApp t1 ty | not $ isVal t1 ->
      let result = eval1 t1
    ...
    TmUnpack x1 x2 t1 t2 | not $ isVal t1 ->
      let result = eval1 t1
      in checkError result $ TmUnpack x1 x2 result t2
    TmPack ty1 t1 ty2 | not $ isVal t1 ->
      let result = eval1 t1
       in checkError result $ TmPack ty1 result ty2
